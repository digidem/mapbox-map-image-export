...
function mapMosaicStream (style, opts) {
  opts = Object.assign({}, DEFAULTS, opts)
  var bbox = parseBboxArg(opts.bbox)

  var token = mapboxgl.accessToken = opts.token || process.env.MAPBOX_TOKEN
  if (!token) {
  ...
  var viewport = calcViewport(map, dpDim, bbox)
  ...
  console.log('ZOOM: %d', viewport.zoom)
  ...
  var maxTileDim = Array(2).fill(Math.min(1500 * pixelRatio, Math.floor(maxRenderBuf / 2 / pixelRatio)))
  map.remove()

  var tiles = getTileStreams(viewport, dpDim, maxTileDim, style)
